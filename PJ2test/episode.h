...
public:
	...
	std::array<int, 4> BagRule() {
    //char a[]=ep_moves[1];
    /*               
    if(step()>=1){     // cout move!!!
    std::cout<< ep_moves[step()-1]<<'\n'; 
    }
    */
    if (step()==0){      
        return {0,0,0,10};      
      ...
      //int playeraction;
      action::place slide = ep_moves[step()-1].code;
      /*
      if(slide.event()=='D'){
           playeraction=0;
      }
      else if(slide.event()=='L'){
           playeraction=1;
      }
      else if(slide.event()=='U'){
           playeraction=2;
      }
      else {
           playeraction=3;
      }          
      std::cout<<"12345"<<slide.event()<<'\n';  
      */
      if ((step()/2)%3==1) {        
		  //if (ep_moves[step()-1].code == action::place(0xb, 0x1))
        action::place place = ep_moves[step()-2].code;
		    if (place.tile() == 1) {
          return {1,0,0,slide.event()};
		    }
		    else if (place.tile() == 2) {
          return {0,1,0,slide.event()};
		    } 
		    else if (place.tile() == 3) {
          return {0,0,1,slide.event()};
		    }           
      }
      else if ((step()/2)%3==2) {        
		  //if (ep_moves[step()-1].code == action::place(0xb, 0x1))
        action::place place = ep_moves[step()-2].code;
        action::place place2 = ep_moves[step()-4].code;
		    if (place.tile() != 1 && place2.tile() != 1) {
          return {0,1,1,slide.event()};
		    }
		    else if (place.tile() != 2 && place2.tile() != 2) {
          return {1,0,1,slide.event()};
		    } 
		    else {
          return {1,1,0,slide.event()};
		    }           
      }
      else{
         return {0,0,0,slide.event()};
      }
    } 
    //if (place.position() == 0xb && place.tile() == 1)
    //std::cout<< ep_moves[step()-1]<<'\n'; 
   //char a[]="B1";
    //int a=3
		//return ep_moves[0];
...
public:
	size_t step(unsigned who = -1u) const {
		...
		switch (who) {
		case action::slide::type: return (size - 9+2) / 2;
		case action::place::type: return (size - (size - 9+2) / 2);
		default:                  return size;
    }
	}
